...
void main() {
  ...
  for (int q = 0; q < 4; q++) {
    ...
    // The bound must be a constant expression. SkSL, which CanvasKit uses on
    // web, will not compile a loop whose limit comes from a uniform: the
    // shader compiler quietly emits a Vulkan stage only, and the asset then
    // throws on web while working perfectly on Android. uRadius decides how
    // much of the fixed window actually contributes.
    for (int i = 0; i <= KUWAHARA_MAX; i++) {
      if (float(i) > uRadius) continue;
      for (int j = 0; j <= KUWAHARA_MAX; j++) {
